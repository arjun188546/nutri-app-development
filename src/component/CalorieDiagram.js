...
const CalorieDiagram = ({ goal, current, remaining }) => {
    ...
    const clampedCurrent = Math.min(current, safeGoal);
    const progress = (clampedCurrent / safeGoal) * circumference;

    // Set stroke color based on goal completion
    const progressColor = current > goal ? "#FF3B30" : "#0072DB";
    const valueColor = current > goal ? "#FF3B30" : "#0072DB";
    ...
    return (
        <View style={styles.container}>
            <View style={styles.diagramContainer}>
                ...
                <View style={styles.textContainer}>
                    <Text
                        style={[styles.remainingText, { color: progressColor }]}
                    >
